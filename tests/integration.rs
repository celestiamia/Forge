...
fn fileio_dev_compiles_and_runs() {
    ...
    let output = Command::new(&bin).output().expect("failed to run fileio binary");
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert_eq!(
        stdout,
...
fn fileio_dev_compiles_and_runs_x86_32() {
    ...
    let output =
        Command::new(&bin).output().expect("failed to run fileio (x86_32) binary");
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert_eq!(
        stdout,
        "file I/O demo\nHello from Forge file I/O!\nfileio ok\n",
...
fn randfile_dev_compiles_and_runs() {
    ...
    let output = Command::new(&bin).output().expect("failed to run randfile binary");
    let stdout = String::from_utf8_lossy(&output.stdout);
    ...
    assert!(stdout.contains("Wrote 10 random numbers."), "unexpected output: {}", stdout);
    ...
    assert!(num_lines >= 10, "expected >=10 number lines, got {}: {}", num_lines, stdout);
    assert!(output.status.success(), "randfile binary exited with non-zero status");
}
...
fn randfile_dev_compiles_and_runs_x86_32() {
    ...
    assert!(stdout.contains("Wrote 10 random numbers."), "unexpected output: {}", stdout);
    assert!(stdout.contains("Done."), "unexpected output: {}", stdout);
    assert!(output.status.success(), "randfile (x86_32) binary exited with non-zero status");
}

#[test]
fn multimod_dev_compiles_and_runs() {
    let bin = compile_source("multimod/multimod.dev", "x86_64-unknown-linux-gnu");
    let output = Command::new(&bin).output().expect("failed to run multimod binary");
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert_eq!(stdout, "multimod ok\n", "multimod produced unexpected output");
    assert!(output.status.success(), "multimod binary exited with non-zero status");
}

#[test]
fn multimod_dev_compiles_and_runs_x86_32() {
    let bin = compile_source("multimod/multimod.dev", "x86_32-unknown-linux-gnu");
    let output = Command::new(&bin).output().expect("failed to run multimod (x86_32) binary");
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert_eq!(stdout, "multimod ok\n", "multimod (x86_32) produced unexpected output");
    assert!(output.status.success(), "multimod (x86_32) binary exited with non-zero status");
}
...
fn multimod_name_conflict_is_reported() {
    ...
    fs::write(out_dir.join("a.dev"), "package a\npub def shared() -> int32:\n    return 1\n")
        .unwrap();
    fs::write(out_dir.join("b.dev"), "package b\npub def shared() -> int32:\n    return 2\n")
        .unwrap();
    fs::write(
...
fn website_dev_compiles_and_serves_pages() {
    ...
    assert!(resp.contains("Forge Website"), "GET / should contain page title");

    // GET /about -> served about.html
    let resp = http_get("/about");
    assert!(resp.contains("200 OK"), "GET /about should return 200 OK");
    assert!(resp.contains("About Forge"), "GET /about should contain title");

    // GET /style.css -> correct content type
    let resp = http_get("/style.css");
    assert!(resp.contains("200 OK"));
    assert!(resp.contains("text/css"), "style.css should have text/css content type");
    assert!(resp.contains("box-sizing"), "style.css should contain CSS rules");

    // GET /app.js -> correct content type
    let resp = http_get("/app.js");
    assert!(resp.contains("200 OK"));
    assert!(resp.contains("application/javascript"), "app.js should have application/javascript content type");

    // GET /api/status -> JSON response
    let resp = http_get("/api/status");
    assert!(resp.contains("200 OK"));
    assert!(resp.contains("application/json"));
    assert!(resp.contains("\"status\":\"ok\""), "API status should contain status:ok");
    assert!(resp.contains("forge-web"), "API status should contain service name");

    // GET /api/random -> JSON with a numeric value
    let resp = http_get("/api/random");
    assert!(resp.contains("200 OK"));
    assert!(resp.contains("application/json"));
    assert!(resp.contains("\"value\":"), "API random should contain value field");

    // GET /hello?name=Forge -> dynamic greeting
    let resp = http_get("/hello?name=Forge");
    assert!(resp.contains("200 OK"));
    assert!(resp.contains("Hello, Forge!"), "Hello page should greet Forge");

    // GET /hello (no name param) -> default to World
    let resp = http_get("/hello");
    assert!(resp.contains("Hello, World!"), "Hello page should default to World");

    // GET /counter -> dynamic counter page
    let resp = http_get("/counter");
    assert!(resp.contains("200 OK"));
    assert!(resp.contains("Total page views:"), "Counter page should show count");

    // GET /nonexistent -> 404
    let resp = http_get("/nonexistent");
    assert!(resp.contains("404 Not Found"), "Unknown route should return 404");
...
fn website_dev_compiles_and_serves_pages_x86_32() {
    ...
    let resp = http_get("/");
    assert!(resp.contains("200 OK"), "GET / should return 200 OK (x86_32)");
    assert!(resp.contains("<html"), "GET / should return HTML (x86_32)");

    let resp = http_get("/api/status");
    assert!(resp.contains("\"status\":\"ok\""), "API status should work (x86_32)");

    let resp = http_get("/nonexistent");
    assert!(resp.contains("404 Not Found"), "Unknown route should return 404 (x86_32)");
