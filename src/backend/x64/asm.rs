...
impl Assembler {
    ...
    pub fn movabs(&mut self, dst: Reg, imm: u64) -> Result<()> {
        encode::mov_r64_imm64(&mut self.buf, dst, imm as i64)
    }
    ...
    pub fn mov(&mut self, dst: impl IntoOp, src: impl IntoOp) -> Result<()> {
        ...
        match (dst, src) {
            ...
            (dst, Operand::Imm64(imm)) => {
                if imm >= i32::MIN as i64 && imm <= i32::MAX as i64 {
                    ...
                    match dst {
                        ...
                        _ => bail!(EncodeError::ImmTooLarge {
                            instruction: "MOV",
                            value: imm,
                        }),
                    }
            ...
            (dst, src) => {
                use crate::backend::error::fmt_operand64;
                bail!(EncodeError::InvalidOperands {
                    instruction: "MOV",
                    dst: fmt_operand64(&dst),
                    src: fmt_operand64(&src),
                });
            }
        }
        Ok(())
    ...
    fn alu(&mut self, op: AluOp, dst: impl IntoOp, src: impl IntoOp) -> Result<()> {
        ...
        match (dst, src) {
            (dst, Operand::Reg(src)) => {
                encode::alu_rm64_r64(&mut self.buf, op, dst, src)?;
            }
            (dst, Operand::Imm32(imm)) => {
                encode::alu_rm64_imm32(&mut self.buf, op, dst, imm)?;
            }
            (dst, Operand::Imm16(imm)) => {
            ...
            _ => {
                bail!(EncodeError::InvalidOperands {
                    instruction: "ALU",
                    dst: fmt_operand64(&dst),
                    src: fmt_operand64(&Operand::Imm8(0)),
                });
            }
        }
        Ok(())
    ...
    pub fn cmp(&mut self, a: impl IntoOp, b: impl IntoOp) -> Result<()> {
        ...
        match (a, b) {
            (dst, Operand::Reg(src)) => {
                encode::cmp_rm64_r64(&mut self.buf, dst, src)?;
            }
            (dst, Operand::Imm32(imm)) => {
                encode::cmp_rm64_imm32(&mut self.buf, dst, imm)?;
            }
            (dst, Operand::Imm16(imm)) => {
    ...
    pub fn test(&mut self, a: impl IntoOp, b: impl IntoOp) -> Result<()> {
        ...
        match (a, b) {
            (dst, Operand::Reg(src)) => {
                encode::test_rm64_r64(&mut self.buf, dst, src)?;
            }
            (dst, Operand::Imm32(imm)) => {
    ...
    pub fn push(&mut self, r: Reg) -> Result<()> {
    ...
    pub fn lea_rip(&mut self, dst: Reg, label: Label) -> Result<()> {
    ...
    pub fn movsxd(&mut self, dst: Reg, src: impl IntoOp) -> Result<()> {
    ...
    pub fn movsd_xmm_xmm(&mut self, dst: Reg, src: Reg) -> Result<()> {
        encode::movsd_xmm_xmm(&mut self.buf, dst, src)
    }

    pub fn movsd_xmm_mem(&mut self, dst: Reg, src: Mem) -> Result<()> {
        encode::movsd_xmm_mem(&mut self.buf, dst, src)
    }

    pub fn movsd_mem_xmm(&mut self, dst: Mem, src: Reg) -> Result<()> {
        encode::movsd_mem_xmm(&mut self.buf, dst, src)
    }

    pub fn addsd(&mut self, dst: Reg, src: Reg) -> Result<()> {
        encode::addsd(&mut self.buf, dst, src)
    }

    pub fn subsd(&mut self, dst: Reg, src: Reg) -> Result<()> {
        encode::subsd(&mut self.buf, dst, src)
    }

    pub fn mulsd(&mut self, dst: Reg, src: Reg) -> Result<()> {
        encode::mulsd(&mut self.buf, dst, src)
    }

    pub fn divsd(&mut self, dst: Reg, src: Reg) -> Result<()> {
        encode::divsd(&mut self.buf, dst, src)
    }

    pub fn cvtsi2sd(&mut self, dst: Reg, src: Reg) -> Result<()> {
        encode::cvtsi2sd(&mut self.buf, dst, src)
    }

    pub fn cvttsd2si(&mut self, dst: Reg, src: Reg) -> Result<()> {
        encode::cvttsd2si(&mut self.buf, dst, src)
    }

    pub fn ucomisd(&mut self, a: Reg, b: Reg) -> Result<()> {
