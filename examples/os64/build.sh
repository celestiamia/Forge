#!/usr/bin/env bash
# Build ForgeOS64 into a bootable 1.44 MB floppy image.
#
...
"$FORGEC" src/boot/loader.dev -o build/loader.raw --linker os64-loader.fld
...
echo "== stage 3: 64-bit kernel (LBA 3-10, loaded at 0x100000) =="
"$FORGEC" src/kernel/kernel.dev -o build/kernel.raw --linker os64-kernel.fld
...
truncate -s 1474560 build/os64.img
dd if=build/boot.bin   of=build/os64.img conv=notrunc status=none
dd if=build/loader.raw of=build/os64.img bs=512 seek=1 conv=notrunc status=none
dd if=build/kernel.raw of=build/os64.img bs=512 seek=3 conv=notrunc status=none
echo "   os64.img: $(stat -c %s build/os64.img) bytes"

if [ "${1:-}" = "run" ]; then
    qemu-system-x86_64 -accel tcg,thread=multi \
        -drive file=build/os64.img,format=raw,if=ide -nographic
fi
