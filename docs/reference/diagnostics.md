...
| Lowering | AST → IR | `function types are not supported in the first milestone`, `could not infer generic arguments for \`id\`` |
| Codegen | Machine code emission | `inline assembly is not implemented in the x64 backend`, `` `_dev_gc_collect` (std.gc) is not supported on the x86_32 target; std.alloc's alloc/free work, but there is no garbage collector `` |
...
- Union/enum types in generic signatures
